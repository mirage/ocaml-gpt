    ...
    (fun gpt ->
      ...
      | Ok gpt ->
          ...
          let partition_buf =
            Bytes.create
              (Int32.to_int gpt.Gpt.num_partition_entries
              * Int32.to_int gpt.Gpt.partition_size)
          in
          let partition_entry_offset =
            Int64.to_int gpt.Gpt.partition_entry_lba * 512
          in
          let () = seek_in ic partition_entry_offset in
          let () =
            really_input ic partition_buf 0 (Bytes.length partition_buf)
          in
          close_in ic;
          ...
          for i = 0 to Int32.to_int gpt.Gpt.num_partition_entries - 1 do
            ...
            let entry_buf =
              Cstruct.sub
                (Cstruct.of_bytes partition_buf)
                entry_offset
                (Int32.to_int gpt.Gpt.partition_size)
            in
